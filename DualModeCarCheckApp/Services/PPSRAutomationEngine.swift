    ...
    private func performCheck(session: LoginWebSession, check: PPSRCheck, sessionId: String = "") async -> CheckOutcome {
        ...
        if verification.found < 6 {
            ...
            if verification.found == 0 {
                check.logs.append(PPSRLogEntry(message: "Healing: waiting 4s for JavaScript-rendered content...", level: .info))
                try? await Task.sleep(for: .seconds(4))
                let retryVerification = await session.verifyFieldsExist()
                if retryVerification.found == 0 {
                    failCheck(check, message: "FATAL: No form fields found after extended wait")
                    await captureScreenshotForCheck(session: session, check: check, step: "no_fields", note: "No fields found", autoResult: .fail)
                    return .connectionFailure
                }
            }
        } else {
